...
| Publishing & distributing a bundle | touching `corpus/publish.py`, `corpus/hub.py`, `save_leiden_portable`, or redistributing a corpus | [REFERENCE_CORPUS_DESIGN.md §6](docs/projects/REFERENCE_CORPUS_DESIGN.md#6-publishing-a-bundle-corpus-publish) — `publish` strips MPD playlist membership, drops the MERIT backbone, de-pickles the Leiden fit, and points the MERIT index's metadata at `index.json`; `push`/`fetch` move it via a HF dataset repo. **A published bundle has no `reducer_2d`, so `place()` returns `coords_2d=None` by design**, and `corpus.embeddings` is memory-mapped lazily — read the row count via `n_embeddings`, not `len(corpus.embeddings)` |
...
| UI visual design | touching `ui/static/style.css` surfaces or panel chrome | [docs/projects/UI_DESIGN_OVERHAUL_PLAN.md](docs/projects/UI_DESIGN_OVERHAUL_PLAN.md) — a four-level **elevation** ladder; depth is surface lightness first, shadow second. **The map deliberately gets no treatment.** Planned, not yet applied; Phase 0 A/B preview at `ui/static/design-preview.html` (throwaway). Two earlier drafts (glassmorphism/neumorphism, then atmospheric light) were built and rejected — **read the note at the top before proposing anything that makes near-black emit light** |
| Mobile UI | touching `ui/static/mobile.js`, the `body.mshell` block in `style.css`, or anything phone-shaped | [docs/ui.md § Mobile shell](docs/ui.md#mobile-shell--one-surface-at-thumb-height) — under 768px the same DOM is **moved** into a bottom sheet, not duplicated, so `app.js` keeps driving it by id. Design source: `ui/Anther Mobile.dc.html` option 1b. **`AtlasGraph`/`AtlasMobile` are global-lexical `const`s, never on `window`** — cross-file guards must use `typeof` |
| Track sources & search tiers | touching `atlas.search`, `atlas.search_albums`, `_relevant`, `anther_ml/spotify_deezer.py` or `anther_ml/itunes.py` | [docs/ui.md](docs/ui.md#search-tiers--why-there-are-four) — the tier gate is relevance-based, not emptiness-based, and tiers **top up** rather than stop at the first hit; **read before "simplifying" it**. Album search is tiered the same way ([§ Album search](docs/ui.md#album-search--the-same-four-tiers-a-different-failure)) — Deezer's album index matches the **title only**, so "king krule the ooz" finds nothing without the artist-discography tier |
