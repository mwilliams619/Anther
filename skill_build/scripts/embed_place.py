"""Embed a discography's previews through MERT+MERIT and place onto the frozen corpus.
Usage: python embed_place.py <csv_path> <out_dir> <out_prefix> [repo]
Runs as a subprocess (live network + local MERT + GPU). Writes
<out_prefix>_placements.csv and <out_prefix>_embeddings.npz into out_dir.
"""
...
csv_path, out_dir, prefix = sys.argv[1], sys.argv[2], sys.argv[3]
LOCAL = "models/mert_v1_330m"
device = "cuda" if torch.cuda.is_available() else "cpu"
processor = Wav2Vec2FeatureExtractor.from_pretrained(LOCAL, trust_remote_code=True)
model = AutoModel.from_pretrained(LOCAL, trust_remote_code=True).to(device).eval()
heads = load_heads(device="cpu")  # merit_query_vector projects on cpu; keep heads there
corpus = ReferenceCorpus.load("models/corpus_mpd_100k_merit")
...
def fetch_decode(url, timeout=30):
    ...
    if len(r.content) < 1024:
        raise ValueError(f"tiny ({len(r.content)}B)")
    raw = tempfile.NamedTemporaryFile(suffix=".m4a", delete=False)
    raw.write(r.content); raw.close()
    try:
...
for i, row in df.iterrows():
    try:
        ...
        neigh = [{"artist": n.get("artist", ""), "title": n.get("title", ""),
                  "score": float(n.get("score", n.get("similarity", 0)))} for n in res["neighbors"][:5]]
        recs.append({
            "track_id": row["track_id"], "title": row["title"], "album": row["album"],
            "era": row["era"], "year": int(row["year"]), "release_date": row["release_date"],
            "cluster_id": int(cl["id"]), "cluster_label": cl["label"], "cluster_conf": float(cl["confidence"]),
        ...
        mert_mat.append(mert_vec); merit_mat.append(mvec)
        if (i + 1) % 10 == 0:
...
out_df.to_csv(os.path.join(out_dir, f"{prefix}_placements.csv"), index=False)
np.savez_compressed(os.path.join(out_dir, f"{prefix}_embeddings.npz"),
