...
df = pd.read_csv("discography_metadata_2new.csv")
...
out_df.to_csv(os.path.join(OUT, "placements_2new.csv"), index=False)
np.savez_compressed(os.path.join(OUT, "embeddings_2new.npz"),
                    mert=np.array(mert_mat, dtype=np.float32),
                    merit=np.array(merit_mat, dtype=np.float32),
                    track_ids=np.array(ids))
with open(os.path.join(OUT, "embed_2new_skips.json"), "w") as f:
